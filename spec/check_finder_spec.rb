...
RSpec.describe CheckFinder do
  ...
  describe '#pawn_search?' do
    context 'when it spots a pawn in a capture position' do
      it 'returns true' do
        board = Array.new(8) { Array.new(8, '') }
        king_position = [4, 4]
        board[3][3] = '♟'
    ...
    context 'when it does not spot a pawn in a capture position' do
      it 'returns false' do
        board = Array.new(8) { Array.new(8, '') }
        king_position = [1, 4]
        color = :white
